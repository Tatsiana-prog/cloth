...
btnSelection1.addEventListener("click", () => {
  const filteredData = originalCards.filter(
    (card) => card.description === "Футер 2-х нитка"
  );
...
btnSelection2.addEventListener("click", () => {
  const filteredData = originalCards.filter(
    (card) => card.description === "Футер 3-х нитка"
  );
...
btnSelection6.addEventListener("click", () => {
  const filteredData = originalCards.filter(
    (card) => card.description === "Ткань для термобелья"
  );
...
btnSelection3.addEventListener("click", () => {
  const filteredData = originalCards.filter(
    (card) => card.description === "Флис односторонний"
  );
...
btnSelection4.addEventListener("click", () => {
  const filteredData = originalCards.filter(
    (card) => card.description === "Флис двуxcторонний"
  );
...
btnSelection5.addEventListener("click", () => {
  const filteredData = originalCards.filter(
    (card) => card.description === "Трикотаж на флисе"
  );
