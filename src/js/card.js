...
// Define the function for rendering the product card
...
// Обработчик событий для клика по блоку с классом 'product-review'
    ...
    fetch('./products.json')  // Путь к JSON файлу
        ...
        .then(data => {
            data.forEach(product => {
                renderProductCard(product);  // Вызываем функцию для рендеринга карточки товара
            });
        })
